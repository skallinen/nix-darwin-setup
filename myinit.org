...
(use-package vterm
  :ensure t
  :custom
  (vterm-shell "/etc/profiles/per-user/samikallinen/bin/zsh")
  :config
...
(set-face-attribute 'default nil :family "RobotoMono Nerd Font") 
...
(with-eval-after-load 'mcp
  (defun srk/mcp-fix-process-coding ()
    (when (boundp 'mcp-server-connections)
      (maphash
       (lambda (_name conn)
         (let ((p (ignore-errors (oref conn process))))
           (when (process-live-p p)
             (set-process-coding-system p 'utf-8-unix 'utf-8-unix)
             ;; (set-process-coding-system p 'no-conversion 'no-conversion)
             )))
       mcp-server-connections)))
  (advice-add 'mcp-hub-start-server     :after (lambda (&rest _) (srk/mcp-fix-process-coding)))
  (advice-add 'mcp-hub-restart-server   :after (lambda (&rest _) (srk/mcp-fix-process-coding)))
  (advice-add 'mcp-hub-start-all-server :after (lambda (&rest _) (srk/mcp-fix-process-coding)))
  (advice-add 'mcp-hub-restart-all-server :after (lambda (&rest _) (srk/mcp-fix-process-coding))))
#+end_src

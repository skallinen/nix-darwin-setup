...
(message "Antigravity Sync Test: %s" (current-time-string))
#+end_src
* Table of contents
- [[*Dummy][Dummy]]
...
(require 'cl-lib)
;; Fix for "function definition is void: concatenate" error in some packages
(defalias 'concatenate 'cl-concatenate)
...
(setq-default mode-line-format
	      (list "%e"
			...
			'(:eval (shell-command-to-string
				 "defaults read ~/Library/Preferences/com.apple.HIToolbox.plist AppleSelectedInputSources | /etc/profiles/per-user/samikallinen/bin/rg -e '\"KeyboardLayout Name\" = \"([^\"]*)\"' --replace '$1' --only-matching --color never | tr -d '\n'"))
